...
from neural_compiler.ops.tagged_ops import evaluate_tagged_op, materialize_quote
...
def _eval_graph_tagged(
    ...
    for nid in order:
        ...
        if node.op_type == "const":
            ...
            if isinstance(val, torch.Tensor) and val.ndim >= 1 and val.shape[-1] == VALUE_DIM:
                values[nid] = val
            else:
...
def evaluate_batched(
    ...
    if graph.uses_tagged_values:
        ...
        from neural_compiler.runtime.tagged_value import VALUE_DIM
...
def _eval_graph_tagged_batched(
    ...
    from neural_compiler.runtime.tagged_value import VALUE_DIM
    from neural_compiler.ops.tagged_ops import ARITH_OPS, COMPARE_OPS, LOGIC_OPS
    ...
    for nid in order:
        ...
        if node.op_type == "const":
            ...
            val = inputs[node.name]
            if isinstance(val, torch.Tensor) and val.shape[-1] == VALUE_DIM:
                values[nid] = val
            else:
                values[nid] = from_scalar(val) if not isinstance(val, torch.Tensor) else make_float(val)
...
def _evaluate_tagged_op_batched(op_type: str, args: list[torch.Tensor]) -> torch.Tensor:
    ...
    if op_type in ("number?", "boolean?", "procedure?"):
        from neural_compiler.runtime.tagged_value import is_number, is_bool, is_closure
